...
int mymp4test(const char* mp4FileName)
{
    ...
    /////////   VIDEO variables
    uint32_t      video_num_samples;
    uint32_t      video_width, video_height;
    double         video_frame_rate;
    uint32_t      video_timescale;
    MP4Duration    video_duration;
    uint8_t      *p_video_config;
    uint32_t      n_video_config_size;
    uint8_t     **pp_sps, **pp_pps;
    uint32_t     *pn_sps, *pn_pps;
    uint8_t      *p_video_sample;
    uint32_t      n_video_sample;
    ...
    uint8_t      *p_audio_config;
    uint32_t      n_audio_config_size;
    uint32_t      audio_sample_max_size;
    ...
    if (video_trId == MP4_INVALID_TRACK_ID) {
        ...
        video_name = (char*)MP4GetTrackMediaDataName(mp4File, video_trId);
        if (strncmp(video_name, "mp4v", 4) == 0)
            printf("\nVideo = MPEG4");
        else if (strncmp(video_name, "h263", 4) == 0)
            printf("\nVideo = H.263");
        else if (strncmp(video_name, "avc1", 4) == 0)
            printf("\nVideo = H.264");
        else
            printf("\nVideo = Unknown");
    }
    ...
    if (audio_trId == MP4_INVALID_TRACK_ID) {
        ...
        audio_name = (char*)MP4GetTrackMediaDataName(mp4File, audio_trId);
    ...
    ////////////////////////////////////
    if (video_trId != MP4_INVALID_TRACK_ID) {
        p_video_sample = (uint8_t *)malloc(video_sample_max_size);
